...
class Joystick(go2_base.Go2Env):
    ...
    def _student_stage2_sampling_profile(self, current_command: jax.Array) -> tuple[jax.Array, jax.Array, jax.Array]:
        """Stage_2 command sampling: full +/- vx, +/- vy, +/- yaw_rate.

        The forward-only stage_1 policy is finetuned in stage_2 with the
        wider, symmetric command distribution stored in
        `self._student_stage2_goal_*` (loaded from
        `configs/course_config.json -> stage_2.student_stage2_goal`). The
        per-channel Bernoulli `keep_prob` controls how often each axis is
        non-zero, so combined commands (e.g. simultaneous vx + yaw_rate) are
        sampled with probability `prod(keep_prob)`.
        """
        del current_command
        return (
